...
SKIP_FLASH_ATTN=0
REQUIRE_CUDA_AVAILABLE="${LINGBOT_V2_REQUIRE_CUDA_AVAILABLE:-1}"

usage() {
  cat <<'USAGE'
Usage: bash tools/create_train_env.sh [--env-name NAME] [--recreate] [--resume] [--flash-attn-wheel PATH] [--skip-flash-attn]
...
Use --skip-flash-attn only when preparing a relocatable environment archive
whose deployment Docker builder will compile and install flash-attn later.
...
assert torch.version.cuda, "Torch was installed without a CUDA runtime"
if os.environ.get("LINGBOT_V2_REQUIRE_CUDA_AVAILABLE", "1") == "1":
...
if [[ "${SKIP_FLASH_ATTN}" == "1" ]]; then
  echo "Deferring flash-attn installation to the deployment image builder."
...
# resolution. Reassert the v2 core pins after installing the local packages.
